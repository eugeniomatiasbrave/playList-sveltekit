...
<div class="flex flex-col w-full h-full p-2">
	<div class="w-full">
		<form
			...
			<Input id="name" label="Nombre del Video" value={form?.data?.name ?? data.project.name} errors={form?.errors?.name} />
			<Input id="tagline" label ="Autor" value={form?.data?.tagline ?? data.project.tagline} errors={form?.errors?.tagline}/>
			<Input id="url" label="URL del Video" value={form?.data?.url ?? data.project.url}
			errors={form?.errors?.url} />
			
			<TextArea id="description"
			label="Project description" value={form?.data?.description ?? data.project.description}
			...
			<div class="w-full max-w-lg pt-3">
				<button type="submit" class="btn btn-info w-full max-w-lg">Guardar cambios</button>
			</div>
		</form>
	</div>
</div>